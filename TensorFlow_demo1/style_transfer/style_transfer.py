...
class VGGNet:
    """BUILDS VGG-16 net structure,
     load parameter from pre-train models
     """
...
vgg16_npy_pyth = "D:/user/Download/vgg16.npy"

content_img_path = './heart.jpg'
stytle_img_path = './heart.jpg'
num_steps = 100
learning_rate =10
...
style_val = read_img(stytle_img_path)

content = tf.placeholder(tf.float32,shape=[1,224,224,3])
stytle = tf.placeholder(tf.float32,shape = [1,224,224,3])
...
stytle_gram = [gram_matrix(feature) for feature in stytle_features]
...
stytle_loss = tf.zeros(1,tf.float32)
for s,s_ in zip(stytle_gram,result_style_gram):
    stytle_loss += tf.reduce_mean((s - s_)** 2,[1,2])

loss = content_loss * lambda_c +stytle_loss * lambda_s
...
with tf.Session() as sess:
    ...
    for step in range (num_steps):
        ...
        =sess.run([loss,content_loss,stytle_loss,train_op],
                 feed_dict = {
                     content:content_val,
                     stytle:style_val
                 })
        print('step: %d, loss_value:%8.4f,content_loss:%8.4f, style:%8.4f'
